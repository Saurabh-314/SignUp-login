...
class Register:
    def __init__(self,root):
        ...
        self.left=ImageTk.PhotoImage(file="images/cr1.jpg")
        ...
        # ------------------- Terms & condition ---------------------------------------------
        self.var_chk=IntVar()
        chk = Checkbutton(frame1,text="I agree the Terms & condition",variable=self.var_chk,onvalue=1,offvalue=0,bg="white",font=("times new roman",12,"bold")).place(x=50,y=380)

        # self.btn_img=ImageTk.PhotoImage(file="images/register.png")
        # btn_register=Button(frame1,image=self.btn_img,bd=0,cursor="hand2",command=self.register_data).place(x=50,y=420,width=150)
        btn_register=Button(frame1,text="SUBMIT",font=("times new roman",20),cursor="hand2",command=self.register_data).place(x=50,y=420,width=150)

        btn_login=Button(self.root,text="Sign In",font=("times new roman",20),cursor="hand2").place(x=170,y=500,width=180)
    ...
    def register_data(self):
        if self.txt_fname.get()=="" or self.txt_lname.get()=="" or self.txt_contact.get()=="" or self.txt_email.get()=="" or self.cmb_quest.get()=="" or self.txt_answer.get()=="" or self.txt_password.get()=="" or self.txt_cpassword.get()=="":
            ...
            try:
                ...
                cur=con.cursor()
                cur.execute("select * from employees whare email=%s",self.txt_email.get())
                row=cur.fetchone()
                # print(row)
                if row!=None:
                    messagebox.showerror("Error","User Already Exists",parent=self.root)
                else:
                    cur.excute("insert into employees(f_name,l_name,contact,email,question,answer,password)values(%s,%s,%s,%s,%s,%s,%s)",
                        (
                            self.txt_fname.get(),
                            self.txt_lname.get(),
